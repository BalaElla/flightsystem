...
def search():
    ...
    print('query:', query)
    conn = get_conn('localhost', 'default_user', 'default_password', 'flight_ticket', 'default_role')
    cursor = conn.cursor()
    cursor.execute(query)
    ...
    cursor.close()
    conn.close()
    return render_template('search2.html', airlines=data)
...
def result():
    ...
    conn.commit()
    conn.close()
    return render_template('search2.html', search_result=data0, airlines=data1)
...
def register_branch():
    ...
    if type == 'Customer':
        ...
        query = 'SELECT * FROM airline'
        print('query:', query)
        conn = get_conn('localhost', 'default_user', 'default_password', 'flight_ticket', 'register_role')
        cursor = conn.cursor()
        cursor.execute(query)
        data = cursor.fetchall()
        print(data)
...
def register_customer():
    ...
    conn = get_conn('localhost', 'root', '', 'flight_ticket', 'register_role')
    conn.autocommit = False
    query = ''
    ...
    if data:
        ...
        cursor = conn.cursor()
        query = ''
        query = 'SELECT airline_name, flight_num, ticket_id, departure_airport, departure_time, arrival_airport, arrival_time, price, status, airplane_id FROM ((customer JOIN purchases ON(customer.email=purchases.customer_email)) NATURAL JOIN ticket) NATURAL JOIN flight WHERE status=\'Upcoming\' AND email=\'{}\''.format(email)
        print('query:', query)
        cursor.execute(query)
        data0 = cursor.fetchall()
        print(data0)
        ...
        if customer_track_from and customer_track_to:
            query = """
                SELECT 
                    ...
                    COALESCE(SUM(f.price), 0) AS spending
                FROM 
                    purchases p
                LEFT JOIN 
                ...
                ORDER BY 
                    month;
            """
            cursor.execute(query, (username[9:], customer_track_from, customer_track_to))
        else:
            query = """
                SELECT 
                    DATE_FORMAT(NOW() - INTERVAL (n.n + 1) MONTH, '%%Y-%%m') AS month,
                    COALESCE(SUM(f.price), 0) AS spending
                FROM 
                    ticket t
                LEFT JOIN 
                ...
                LEFT JOIN 
                    flight f ON t.flight_num = f.flight_num
                CROSS JOIN 
                    (SELECT 0 AS n UNION SELECT 1 UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5) AS n
                WHERE 
                    p.customer_email = %s
                    AND DATE_FORMAT(p.purchase_date, '%%Y-%%m') = DATE_FORMAT(NOW() - INTERVAL (n.n + 1) MONTH, '%%Y-%%m')
                GROUP BY 
                    DATE_FORMAT(NOW() - INTERVAL (n.n + 1) MONTH, '%%Y-%%m')
                ORDER BY 
                    month DESC;
            """
            cursor.execute(query, (username[9:],))
...
def customer_result():
    ...
    print(data1)
    if username and password:
        query = 'SELECT airline_name, flight_num, ticket_id, departure_airport, departure_time, arrival_airport, arrival_time, price, status, airplane_id FROM ((customer JOIN purchases ON(customer.email=purchases.customer_email)) NATURAL JOIN ticket) NATURAL JOIN flight WHERE status=\'Upcoming\' AND email=\'{}\''.format(username[9:])
        print('query:', query)
        cursor.execute(query)
...
#     print(request.form)  # 添加这行来查看接收到的表单数据
...
@app.route('/customer/purchase', methods=['GET', 'POST'])
def customer_purchase():
    ticket_choice = request.form.get('ticket_choice')
    if ticket_choice:
        airline_name, flight_num = ticket_choice.split(',')
    # ticket_info = list(request.form.keys())[0].split(',')
    # airline_name = ticket_info[0]
    # flight_num = ticket_info[1]
        ...
        if username and password:
            ...
            WHERE t.airline_name = %s AND t.flight_num = %s AND p.ticket_id IS NULL
            '''
            cursor.execute(query, (airline_name, flight_num))
            data = cursor.fetchall()
            print(data)
...
@app.route('/customer/purchase_result', methods=['POST'])
def customer_purchase_result():
    ...
    if username and password:
        ...
        cursor.execute(query, (ticket_id, username[9:], today))

        query = 'SELECT email, name FROM customer WHERE email=%s'
        cursor.execute(query, (username[9:],))
        data0 = cursor.fetchone()
        print(data0)

        query = 'SELECT ticket_id, airline_name, flight_num FROM ticket WHERE ticket_id=%s'
...
def register_agent():
    ...
    if data:
        ...
        data0 = cursor.fetchall()
        print(data0)
        query = 'SELECT customer_email, airline_name, flight_num, ticket_id, departure_airport, departure_time, arrival_airport, arrival_time, price, status, airplane_id FROM ((purchases NATURAL JOIN booking_agent) NATURAL JOIN ticket) NATURAL JOIN flight WHERE status=\'Upcoming\' AND email=\'{}\''.format(email)
        print('query:', query)
        cursor.execute(query)
        data1 = cursor.fetchall()
        print(data1)
    ...
    if username and password:
        ...
        if agent_commission_from and agent_commission_to:
            query = """
                ...
                WHERE email = %s AND purchase_date >= %s AND purchase_date <= %s
                GROUP BY customer_email
                ORDER BY COUNT(ticket_id)
                LIMIT 5;
            """
            cursor.execute(query, (username[6:], agent_commission_from, agent_commission_to))
            
        else:
            query = """
                SELECT customer_email AS name, COUNT(ticket_id) AS number_of_tickets
                FROM ((booking_agent NATURAL JOIN purchases) NATURAL JOIN ticket) NATURAL JOIN flight
                WHERE email = %s AND period_diff(date_format(now(), '%%Y%%m'), date_format(purchase_date, '%%Y%%m')) < 6
                GROUP BY customer_email
                ORDER BY COUNT(ticket_id)
                LIMIT 5;
            """
            # cursor.execute(query, (username[6:], agent_commission_from, agent_commission_to))
            cursor.execute(query, (username[6:],))

        data = cursor.fetchall()
        for row in data:
            row['number_of_tickets'] = float(row['number_of_tickets'])
...
def customer_data_commission():
    ...
    if username and password:
        ...
        if agent_commission_from and agent_commission_to:
            # Query for agent_customer_commission_ranged
            query = """
                SELECT customer_email AS name, SUM(price)*0.05 AS spending
                FROM ((booking_agent NATURAL JOIN purchases) NATURAL JOIN ticket) NATURAL JOIN flight
                WHERE email = %s AND purchase_date >= %s AND purchase_date <= %s
                GROUP BY customer_email
                ORDER BY SUM(price)*0.05 DESC
                LIMIT 5;
            """
            cursor.execute(query, (email, agent_commission_from, agent_commission_to))
        else:
            # Query for agent_customer_commission
            query = """
                SELECT customer_email AS name, SUM(price)*0.05 AS spending
                FROM ((booking_agent NATURAL JOIN purchases) NATURAL JOIN ticket) NATURAL JOIN flight
                WHERE email = %s AND period_diff(date_format(now(), '%%Y%%m'), date_format(purchase_date, '%%Y%%m')) <= 12
                GROUP BY customer_email
                ORDER BY SUM(price)*0.05 DESC
                LIMIT 5;
            """
...
def agent_commission_result():
    ...
    if username and password:
        ...
        # 查询即将到来的航班信息
        query = """
            SELECT customer_email, airline_name, flight_num, departure_airport, 
                   departure_time, arrival_airport, arrival_time, price, status, airplane_id 
            FROM ((purchases NATURAL JOIN booking_agent) NATURAL JOIN ticket) 
            NATURAL JOIN flight 
            WHERE status='Upcoming' AND email=%s
        """
        print('query:', query)
        cursor.execute(query, (email,))
        data1 = cursor.fetchall()
        ...
        if commission_from and commission_to:
            # 查询指定日期范围内的佣金数据
            query = """
                SELECT sum(price) * 0.05 AS total_commission, 
                       avg(price) * 0.05 AS avg_commission, 
                       COUNT(ticket_id) AS total_tickets 
                FROM ((booking_agent NATURAL JOIN purchases) NATURAL JOIN ticket) 
                     NATURAL JOIN flight 
                WHERE purchase_date >= %s AND purchase_date <= %s AND email = %s
            """
            print('query:', query)
            cursor.execute(query, (commission_from, commission_to, email))
            data = cursor.fetchone()
            print(data)
            total_commission = float(data.get('total_commission')) if data.get('total_commission') else None
            average_commission = float(data.get('avg_commission')) if data.get('avg_commission') else None
            ...
            # 查询最近30天的佣金数据
            query = """
                SELECT sum(price) * 0.1 AS total_commission, 
                       avg(price) * 0.1 AS avg_commission, 
                       COUNT(ticket_id) AS total_tickets 
                FROM ((booking_agent NATURAL JOIN purchases) NATURAL JOIN ticket) 
                     NATURAL JOIN flight 
                WHERE datediff(now(), purchase_date) <= 30 AND email = %s
            """
            print('query:', query)
            cursor.execute(query, (email,))
            data = cursor.fetchone()
...
def agent_purchase_ticket():
    ...
    if username and password and airline_name and flight_num:
        ...
        conn.autocommit = False
        cursor = conn.cursor()
        query = 'SELECT airline_name FROM booking_agent_work_for WHERE email = %s AND airline_name = %s'
        print('query:', query)
        cursor.execute(query, (username[6:], airline_name))
        data = cursor.fetchone()
        print(data)
        if data:
            query = 'SELECT email FROM customer WHERE email = %s'
            print('query:', query)
            cursor.execute(query, (agent_customer_email,))
            data = cursor.fetchone()
            print(data)
            if data:
                # Replace the stored procedure call with a plain query
                query = """
                    SELECT ticket_id
                    FROM ticket
                    WHERE airline_name = %s AND flight_num = %s AND ticket_id NOT IN (
                        SELECT ticket_id
                        FROM purchases
                        WHERE 1
                    )
...
def agent_purchase_register_cont():
    ...
    if data:
        ...
        data = cursor.fetchone()
        print(data)
        password_hashed = data['md5(%s)' % password]
        
        query = 'INSERT INTO customer VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)'
        cursor.execute(query, (email, name, password_hashed, building_number, street, city, state, phone_number, passport_number, passport_expiration, passport_country, date_of_birth))
        print('query:', query)
        
        query = 'CREATE USER %s@localhost IDENTIFIED BY %s'
        print('query:', query)
        cursor.execute(query, (f'customer_{email}', password))
        
        query = 'GRANT customer_role TO %s@localhost'
        print('query:', query)
        cursor.execute(query, (f'customer_{email}',))
...
def register_staff():
    ...
    if data:
        ...
        # Top 5 customers by ticket count for the last year
        query = """
            WITH A AS (
                ...
                WHERE username = %s AND date_format((now()-INTERVAL 1 year), '%%Y') = date_format(purchase_date, '%%Y')
                GROUP BY customer_email
            )
...
def revenue_data_month():
    ...
    if username and password:
        ...
        query = """
            ...
            WHERE date_format((now()-INTERVAL 1 month), '%%Y-%%m') = date_format(purchase_date, '%%Y-%%m')
...
def revenue_data_year():
    ...
    if username and password:
        ...
        query = """
            SELECT SUM(price) AS revenue
            FROM (purchases NATURAL JOIN ticket) NATURAL JOIN flight
            WHERE date_format((now()-INTERVAL 1 year), '%%Y') = date_format(purchase_date, '%%Y')
                AND booking_agent_id IS NULL
                AND airline_name IN (
                    SELECT airline_name
                    FROM airline_staff
...
def ticket_sold_data():
    ...
    if username and password:
        ...
        if staff_ticket_from and staff_ticket_to:
            # staff_ticket_stats_ranged equivalent query
            query = """
                WITH A AS (
                    SELECT date_format((now() - INTERVAL (cast(help_topic_id as signed) - 1) month), '%%Y-%%m') AS month
                    FROM mysql.help_topic
                    WHERE date_format((now() - INTERVAL (cast(help_topic_id as signed) - 1) month), '%%Y-%%m') >= date_format(%s, '%%Y-%%m')
                        AND date_format((now() - INTERVAL (cast(help_topic_id as signed) - 1) month), '%%Y-%%m') <= date_format(%s, '%%Y-%%m')
                ),
                B AS (
                    SELECT date_format(purchase_date, '%%Y-%%m') AS month, COUNT(ticket_id) AS num_ticket_bar
                    FROM (purchases NATURAL JOIN ticket) NATURAL JOIN flight
                    WHERE date_format(purchase_date, '%%Y-%%m') >= date_format(%s, '%%Y-%%m')
                        AND date_format(purchase_date, '%%Y-%%m') <= date_format(%s, '%%Y-%%m')
                        AND airline_name IN (
                            SELECT airline_name
                            FROM airline_staff
                            WHERE username = %s
            ...
            """
            print('query:', query)
            cursor.execute(query, (staff_ticket_from, staff_ticket_to, staff_ticket_from, staff_ticket_to, username[6:]))
        else:
            # staff_ticket_stats_recent equivalent query
            query = """
                WITH A AS (
                    SELECT date_format((now() - INTERVAL (2) year + INTERVAL (cast(help_topic_id as signed) - 1) month), '%%Y-%%m') AS month
                    FROM mysql.help_topic
                    WHERE date_format((now() - INTERVAL (2) year + INTERVAL (cast(help_topic_id as signed) - 1) month), '%%Y') = date_format((now() - INTERVAL (1) year), '%%Y')
                ),
                B AS (
                    SELECT date_format(purchase_date, '%%Y-%%m') AS month, COUNT(ticket_id) AS num_ticket_bar
                    FROM (purchases NATURAL JOIN ticket) NATURAL JOIN flight
                    WHERE date_format(purchase_date, '%%Y') = date_format((now() - INTERVAL (1) year), '%%Y')
                        AND airline_name IN (
                            SELECT airline_name
                            FROM airline_staff
                            WHERE username = %s
...
def staff_flight_update():
    ...
    if username and password:
        ...
        WHERE username = %s 
        AND datediff(departure_time, now()) <= 30
        '''
        cursor.execute(query, (username,))
        print('query:', query)
        # cursor.execute(query)
...
def staff_track_report():
    ...
    if username and password:
        ...
        # query = 'SELECT COUNT(ticket_id) AS amount FROM (purchases NATURAL JOIN ticket) NATURAL JOIN flight WHERE date_format(purchase_date, \'%Y\')=date_format((now() - INTERVAL (1) year), \'%Y\') AND airline_name=\'{}\''.format(airline_name)
        query = '''
        SELECT COUNT(ticket_id) AS amount
        FROM (purchases NATURAL JOIN ticket) NATURAL JOIN flight
        WHERE YEAR(purchase_date) = YEAR(CURDATE() - INTERVAL 1 YEAR)
        AND airline_name = %s
        '''
        ...
        SELECT COUNT(ticket_id) AS amount
        FROM (purchases NATURAL JOIN ticket) NATURAL JOIN flight
        WHERE date_format(purchase_date, '%%Y-%%m') = date_format(now() - INTERVAL 1 MONTH, '%%Y-%%m')
        AND airline_name = %s
        '''
        cursor.execute(query, (airline_name,))
        print('query:', query)
        ...
        ticketsold_month = data['amount']
         # Tickets sold within the given range
        query = """
        SELECT COUNT(ticket_id) AS amount 
        FROM (purchases NATURAL JOIN ticket) NATURAL JOIN flight 
        WHERE date_format(purchase_date, '%%Y-%%m') >= date_format(%s, '%%Y-%%m') 
    ...
    """
        print('query:', query)
        cursor.execute(query, (staff_ticket_from, staff_ticket_to, username))
        data = cursor.fetchone()
        print(data)
...
def staff_admin_new_flights():
    ...
    if username and password:
        ...
        if data:
            ...
            query = 'SELECT a.airport_name FROM airport AS a, airport AS b WHERE a.airport_name=%s AND b.airport_name=%s'
            print('query:', query)
            ...
            # Check for time conflict with existing flights
            query = """
                SELECT airline_name
                FROM flight
                WHERE airline_name = %s AND airplane_id = %s
                AND (
                    (%s >= departure_time AND %s <= arrival_time) OR 
                    (%s >= departure_time AND %s <= arrival_time) OR 
                    (departure_time >= %s AND departure_time <= %s) OR 
                    (arrival_time >= %s AND arrival_time <= %s)
                )
            """
            print('query:', query)
            cursor.execute(query, (airline_name, plane_id, departure_time, departure_time, arrival_time, arrival_time, departure_time, arrival_time, departure_time, arrival_time))
            data = cursor.fetchone()
            print(data)
            if data:
                cursor.close()
                ...
                """
                print('query:', query)
                cursor.execute(query, (airline_name, flight_num, departure_airport, departure_time, arrival_airport, arrival_time, price, status, plane_id))
                ...
                # Insert tickets for the new flight
                for i in range(seats):
                    query = 'INSERT INTO ticket VALUES (%s, %s, %s)'
                    print('query:', query)
                    cursor.execute(query, (ticket_id_start + i, airline_name, flight_num))
